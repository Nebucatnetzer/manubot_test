...
header-includes: '<!--
  ...
  <link rel="alternate" type="text/html" href="https://Nebucatnetzer.github.io/manubot_test/v/785fd9d6259154c0f3e685e5b0d943b6243da0a7/" />

  <meta name="manubot_html_url_versioned" content="https://Nebucatnetzer.github.io/manubot_test/v/785fd9d6259154c0f3e685e5b0d943b6243da0a7/" />

  <meta name="manubot_pdf_url_versioned" content="https://Nebucatnetzer.github.io/manubot_test/v/785fd9d6259154c0f3e685e5b0d943b6243da0a7/manuscript.pdf" />
...
<small><em>
This manuscript
([permalink](https://Nebucatnetzer.github.io/manubot_test/v/785fd9d6259154c0f3e685e5b0d943b6243da0a7/))
was automatically generated
from [Nebucatnetzer/manubot_test@785fd9d](https://github.com/Nebucatnetzer/manubot_test/tree/785fd9d6259154c0f3e685e5b0d943b6243da0a7)
on December 26, 2019.
</em></small>
...
selbst unter der GPL Version 3 veröffentlicht wird. Wenn möglich soll
...
| Ziel-Nr. <!-- style="min-width: 200px" -->| Zielsetzung | Muss <!-- style="min-width: 30px" -->| Wunsch<br> (1-5, 5=sehr wichtig) |
|:----------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------|:------|:----------------------------------|
...
Abbildung:(@fig:context) zu sehen ist, werden die Aktionen effektiv immer vom BorgBackup Binary
...
Benutzung des Textsatzsystems TeXs vereinfacht. LaTeX wurde gegenüber einem
„What You See Is What You Get“ (z.Bsp. MS. Word) Editor gewählt, weil es einem
mit seiner Markup Sprache erlaubt das Dokument in Textdateien zu erstellen,
...
dem Buch „Practical Typography“ von Matthew Butterick
[@YTQxX4eF] angewandt. Bei den Diagrammen wurden
...
einander gegenübergestellt und grössere Abweichungen begründet. Die Nummer vor
...
Projektleiters), ergeben sich gemäss der Berechnung in der Tabelle:(@tbl:costs),
theoretische Kosten von 19080 CHF für die Umsetzung dieser Arbeit. Die Kosten
...
betrachten und zu bewerten. Auf Grundlage der Analyseergebnisse werden
erforderliche Massnahmen zur Gestaltung der Umweltbeziehungen abgeleitet. Die
Gestaltung der Projektumweltbeziehungen ist eine Projektmanagementaufgabe. In
der Tabelle:(@tbl:umweltanalyse) wurden die Anforderungen und Wünsche mit Einschätzung der
Wahrscheinlichkeit und der Einflussnahme aufgenommen. Zusätzlich ist die
Beziehung der Stakeholder zum Projekt noch in der Abbildung:(@fig:stakeholder) grafisch
dargestellt.
...
Tabelle:(@tbl:risikobewertung-wahrscheinlichkeit) nach der Wahrscheinlichkeit des Eintreffens bewertet und
entsprechend der Tabelle:(@tbl:risikobewertung-auswirkung) nach seiner Auswirkung im Bezug auf die
Nützlichkeit der gemachten Backups.

In der Tabelle:(@tbl:risikobeschreibung) sind dabei die Risiken für das Szenario aufgelistet und
nummeriert. In der Abbildung:(@fig:currentrisk) ist die Bewertung des Ist-Risikos grafisch
dargestellt und in der Abbildung:(@fig:targetrisk) ist das Soll-Risiko, welches mit dieser
...
erreichen. Dabei beschreibt er, was beim Versuch der Zielerreichung passieren
kann. Je nach Ablauf kann auch ein Fehlschlag ein Ergebnis eines
Anwendungsfalls sein (e.g. falsches Passwort beim Login). Dabei wird die
...
des Verhaltens von Software- und anderen Systemen. Es stellt Anwendungsfälle
...
Die nachfolgend aufgeführten Use Cases, Tabellen:(@tbl:backup, @tbl:delete, @tbl:restore, @tbl:restore-file, @tbl:mount, @tbl:settings, @tbl:automatic)
wurden dem Anwendungsfalldiagramm, Abbildung:(@fig:usecase), entnommen und zusätzlich noch
um jeweils ein Aktivitätsdiagramm, Abbildungen: (@fig:backup, @fig:delete, @fig:restore, @fig:mount, @fig:settings, @fig:automatic),
erweitert um den Ablauf verständlicher zu machen.
...
zusammen. Dadurch ergeben sich dann die Bewertungen, welche in der Tabelle:(@tbl:example-table)
...
entsprechend der Bewertung in der Tabelle Projektziele:(@tbl:projektziele).
...
den Code über Projekte hinaus einheitlich aussehen lässt. Zudem hilft hier auch
Visual Studio stark den Code entsprechend zu formatieren. Besonders angenehm
sind die Klassen- und Methodennamen der offiziellen Frameworks. Insgesamt sehr
gut gelöst aber in Sachen Lesbarkeit noch etwas hinter Python.
...
ISO/IEC 14882:2017 [@5QhL5DLO] ist zurzeit
die aktuellste Variante. Die Sprache existiert seit ca. 33 Jahren und hat eine
weitreichende Verbreitung gefunden. C++ ist auf allen Betriebssystemen gut
unterstützt muss jedoch für jedes System separat kompiliert werden.
...
wie der Code am besten ausschauen sollte. Somit haben sich über die Jahre
mehrere Standards etabliert.
...
[@d6rnuGOy] und wird durch die Python
Software Foundation in einem Community basierten Modell entwickelt.
...
Python unterstützt mehrere Programmierungsparadigmen wie etwa objektorientiert,
funktionale oder prozedurale Paradigmen. Bei der Entwicklung von Python wurde
sehr grossen Wert auf die Lesbarkeit der Sprache gelegt. Dies mit dem
Hintergedanken das eine Programmiersprache viel häufiger gelesen als effektiv
geschrieben wird [@MmuEtWF6].
...
einfachen Befehl, Code Snippet:([1](#code-example)) starten.
...
dass man bei Variablen nicht explizit den Typ angeben muss und die Programme
zur Laufzeit für den Computer übersetzt werden. Interpretierte Sprachen haben
den Vorteil, dass man mit ihnen in der Regel sehr schnell und unkompliziert
...
Systemen ohne grosse Änderungen laufen sollen und sich dabei soweit als möglich
wie eine native Applikation verhalten und „anfühlen“ soll.
...
unterstützten Systemen, oftmals sehen die Applikationen jedoch eher wie eine
Webseite als wie eine Desktop Applikation aus. Ein weiterer Nachteil ist der
hohe Ressourcenverbrauch, da jede Applikation nahezu einer eigenen Instanz des
Google Chrome Browsers gleich kommt.
...
die Testfälle in der Tabelle:(@tbl:testcases), erstellt. Diese werden final von Hand
überprüft. Somit kann vermieden werden, dass nicht funktionierende automatische
Tests den Abschluss des Projektes verhindern. Da die Testfälle sich
hauptsächlich an den Use Cases orientieren, gibt es ein paar Ziele die, dadurch
nicht getestet werden können. Zudem sind zurzeit nur ca. 20 der Ziele durch die
Use Cases abgedeckt. Die weiteren Ziele lassen sich erst sinnvoll integrieren,
wenn die Basis für das Programm geschaffen wurde. Somit werden diese Ziele erst
im Anschluss zur Diplomarbeit umgesetzt.
...
[@mgdc6bIz] Linux, mit der GNOME Desktop Umgebung
[@ANdMtja9], als Betriebssystem. Die Tests werden jeweils
gegen eine von PyInstaller generierte Binärdatei ausgeführt. Der genaue Vorgang
der Erstellung dieser Datei wird in der Sektion: [Releases] beschrieben. Somit
...
basiert auf dem UML Standard. Im Diagramm wurden nicht alle „Properties“ und
Methoden alles Klassen aufgezeichnet, sondern nur solche, die auf eine andere
Klasse verweisen. Dadurch bleibt das Diagramm übersichtlicher. Die
Klassennamen, welche in fetter Schrift gehalten sind, wurden dabei vom
...
bewerten die Probanden dann die einzelnen Aufgaben nach ihrer Schwierigkeit,
Tabelle:(@tbl:usability). Daraus lässt sich dann eine sogenannte Heatmap erstellen. Aus der
Heatmap kann man anschaulich herauslesen, welche Bereiche für die User noch zu
kompliziert sind und Nacharbeit benötigen.
...
Angaben in der Tabelle:(@tbl:probanden) sind jedoch die Selbsteinschätzung der Probanden
...
2. Du hast aus Versehen die Datei `/home/testuser/Downloads/Example.svg`
...
vertraut ist. Als zusätzlichen Input gab er an, dass ein Kontextmenü, welches
sich mit Rechtsklick auf ein Element öffnet, etwas sei was er gerne hätte, da
er andere Anwendungen oft so steuert. Aufgabe 5 war auch etwas herausfordernder
...
verloren gefühlt und hätte sich auch ein Kontextmenü auf dem Rechtsklick
...
wird in die künftige Weiterentwicklung der Applikation eingepflegt. Aus
Ressourcengründen allerdings erst nach der Diplomarbeit.
...
Das übersichtliche Userinterface in Abbildung:(@fig:bit), wurde für Borg-Qt als
...
Die angenehmere Variante ist es den Qt Designer, Abbildung:(@fig:designer), zu nutzen. Mit
diesem lassen sich die Oberflächen in einer grafischen Oberfläche designen und
auch gleich starten. Damit ist direkt zu sehen wie sich die Oberflächen auf dem
System verhalten.

![Ein Screenshot der Applikation Qt Designer](images/qt_designer.png){#fig:designer width="90%"}

Mit der ersten GUI Version wurden die ersten
Basisziele der Projektarbeit umgesetzt. Im Hauptfenster, Abbildung:(@fig:borgqt-main1),
...
zur Auswahl des zu sichernden Pfades. Im oberen Bereich findet sich die Toolbar
mit den Aktionen, die der User ausführen kann. Gemäss den Use Cases sind dies
„Backup, Restore, Mount, Delete und Settings“.

Bei den Icons wurde zuerst versucht diese nach der „Icon Naming Specification“
[@7jYaon8r]
auszuwählen. Diese Spezifikationen würden es erlauben einfach den definierten
Namen des Icons anzugeben. Qt würde dann jeweils das passende Icon basierend
auf dem System anzeigen. Somit wären die Icons passend zum jeweiligen
...
Abbildung:(@fig:settings-include), kann der User die Ordner und Dateien auswählen, die er sichern
...
1](images/borgqt_settings_general_v1.png){#fig:settings-general}

![Screenshot der Borg-Qt „Include“ Einstellungen Version
1](images/borgqt_settings_include_v1.png){#fig:settings-include}

![Screenshot der Borg-Qt „Exclude“ Einstellungen Version
1](images/borgqt_settings_exclude_v1.png){#fig:settings-exclude}
...
1](images/borgqt_progress_v1.png){#fig:progress}
...
Einstellungen sollen in einer Klar-Text Datei gespeichert werden. Dies hat zum
einen den Vorteil, dass man die Einstellungen sehr einfach sichern kann. Zum
...
Modul `configparser`
[@EdMlwJHH]
verwendet. Dieses macht es einem sehr einfach eine Datei im „INI“ Stil zu
erstellen und parsen.

„INI“ Stil bedeutet dabei das die Einstellungen in „Key/Value“ Paaren
gespeichert werden. Somit kann man einfach auf den benötigten Wert zugreifen,
in dem man seinen Schlüssel angibt. Ein Beispiel ist im Code Snippet:([2](#code-init)) zu
sehen.
...
Abhilfe schaffte hier ein Stackexchange Post [@fa82c0xs]. Dieser schlug vor,
...
als Methode der `Config` Klasse, Code Snippet:([3](#code-config)), implementiert. Somit muss
...
`_get_path`, Codesnippet:([4](#code-get-path)). Es gibt dabei zwei mögliche Pfade, wo die
Konfigurationsdatei liegen könnte. Befindet sich die Datei nicht am vorgegeben
Pfad `~/.config/borg_qt/borg_qt.conf` oder direkt „neben“ dem Binary, gibt die
Applikation eine entsprechende Meldung, Abbildung:(@fig:missing-config), aus. Der Hauptpfad
...
aus der Konfigurationsdatei grafisch dar und übergibt allenfalls geänderte
Werte ans Backend, welches die Konfiguration, dann wieder in der Datei
speichert.
...
Codesnippet:([5](#code-config-list)) aus. Dabei wird jedes Element einzeln aus dem `QListWidget`
...
[@1B2w4mlEN], mit. In der Praxis
lies sich der Fortschrittsdialog und der Thread jedoch nicht so verknüpfen das
sich der Dialog schliesst, wenn das Backup durchgelaufen ist und der Thread
wieder entfernt wird. Aus diesem Grund wurde dann ein erfolgreicher Test mit
dem PyQt Modul `QThread` [@PyQ7veZ4] gemacht.
Nach Beendigung des Backups wird der Fortschrittsdialog automatisch
...
an `json_error` weitergegeben. Mittels der Methode im Codesnippet:([6](#code-json-err)), wird die
...
separaten Fenster ausgegeben. Die Methode wurde dabei auf der Klasse
`BorgQtThread` umgesetzt und steht somit allen Funktionen zur Verfügung. Die
Fehlermeldung bei einer fehlenden Konfigurationsdatei, Abbildung:(@fig:missing-config),
...
instantiieren 3 Argumente auf: `includes`, `excludes`, `prefix`. Wobei
`excludes` und `prefix` beide optional sind. Im Hauptcode werden diese
Argumente aus der Konfigurationsdatei ausgelesen und übergeben. Die Includes
werden im Falle eines Backups im Hintergrund aus der Konfigurationsdatei
gelesen. Wenn es der User manuell ausführt, wird der im Frontend ausgewählte
Pfad mitgegeben.
...
einen String übergibt, sind die zusätzlichen Anführungszeichen nicht notwendig
und führen sogar dazu, dass die Pfade gar nicht funktionieren. Somit werden die
„Excludes“ mittels der Methode `_process_excludes` mit dem entsprechenden
Parameter gepaart und als gesamte Liste an das finale Kommando angehängt. Die
„Includes“ funktionieren auf die gleiche Weise, benötigen jedoch keine
zusätzlichen Parameter. Zu sehen ist dies im Codesnippet:([7](#code-create)).
...
Dialogs.](images/borgqt_progress_v2.png){#fig:progress-v2}
...
diesem Grund wurde der Fortschrittsbalken mit Prozentanzeige durch einen sich
...
Abbildung:(@fig:archive-list), grün respektive rot umrahmt. Für die beiden Funktionen wurde
...
Stackoverflow [@1FMRL8454] übernommen.
...
[@6LhuS78I]. BorgBackup kann mehrere Archive gleichzeitig mounten. Der User
...
erscheint folgende Fehlermeldung, Abbildung:(@fig:no-archive), um den Benutzer darauf
...
erscheint die Fehlermeldung, Abbildung:(@fig:not-writeable), und der Vorgang wird abgebrochen.
...
ist.](images/borgqt_not_writeable.png){#fig:not-writeable}
...
Genau dies wird in der Applikation auch so überprüft. Hat die Applikation
Schreibrechte auf den Zielpfad, wird das ausgewählte Archiv auf diesem Pfad
gemountet. Anschliessend wird der Pfad in einem Dateimanager geöffnet, damit
der Benutzer direkt mit den Dateien weiterarbeiten kann. Wurde erkannt, dass
das Archiv bereits gemountet wurde, also der Pfad nicht schreibbar ist, öffnet
die Applikation direkt den Dateimanager, ohne zu versuchen das Archiv noch
einmal zu mounten.
...
Applikation dem Benutzer einen Dialog, Abbildung:(@fig:yes-or-no). Bestätigt er diesen mit
„Yes“ wird der Vorgang fortgesetzt und das Archiv gelöscht mit „No“ wird der
...
Funktion.](images/borgqt_yes_no.png){#fig:yes-or-no}
...
die dazugehörigen Prozesse zu starten und zu verwalten
[@1A5Nmo0JV]. Die Prozesse werden über
sogenannte „Services“ gestartet. Die Services werden dabei einfach in
Klartextdateien mit der Dateiendung `.service` definiert. Der Inhalt orientiert
sich dabei praktischerweise am „INI“ Stil. In Borg-Qt wurde das INI Format
...
versteht eine Vielzahl an Datumsformaten
[@H0CHfZwD]. In Borg-Qt
...
Abbildung:(@fig:settings-schedule). Die Predefined Option wird dabei in die von Systemd
...
Wochentagen zu ermöglichen, damit der Benutzer etwa folgenden Zeitplan
erstellen könnte „Montag, Mittwoch, Freitag stündliche Backups.“ (`Monday,
Wednesday, Friday *-*-* *:00:00`).

![Screenshot der „Schedule“
Einstellungen](images/borgqt_settings_schedule.png){#fig:settings-schedule}
...
respektive Timers wurde eine Klasse `SystemdFile` erstellt. Somit könnte die
Funktion auch einfach in einem anderen Projekt verwendet werden.
...
abgelegt wird, wurde das Unix Tool `which` verwendet, um den exakten
Speicherort zu erhalten. Mittels des Befehls `which borg-qt` erhält man den
absoluten Speicherort der Datei. Zusammen mit den Daten aus den Einstellungen
wird diese Information in einem `Configparser` Objekt gespeichert, welches dann
mithilfe der `SystemdFile` Klasse in eine `borg_qt.service`, Codesnippet:([8](#code-service)),
respektive `borg_qt.timer`, Codesnippet:([9](#code-timer)), Datei, im Systemd Pfad
...
![Screenshot der Borg-Qt „Exclude“ Einstellungen Version
2](images/borgqt_settings_exclude_v2.png){#fig:settings-exclude-v2}
...
angezeigt. Es gibt ihm einen kurzen Überblick welcher Button welche Aktion
auslöst und welche Elemente welche Information anzeigen. Optional kann der
Benutzer entscheiden, dass er das Fenster beim nächsten Start nicht mehr
...
Das Binary wird mit dem Programm
„PyInstaller“[@C2Jz1HGm] erstellt. PyInstaller wird auf
der Kommandozeile mit Angabe der Hauptdatei des Codes ausgeführt. Der Befehl
dafür ist relativ einfach, Codesnippet:([10](#code-install)). Pfade zusätzlicher Dateien wie
etwa Icons müssen mit angegeben werden. PyInstaller kann diese sonst nicht in
das Binary einbinden. Der gezeigte Code wurde dabei in ein Makefile
...
Ziel-Nr. in der Tabelle:(@tbl:projektziele). In der Spalte Bemerkung wird noch kurz ausgeführt,
...
Betriebssystem getestet mit welchem die Probanden bereits etwas Erfahrung
haben. Zwei der Probanden waren ab dem Verhalten und Aussehen des Dateimanagers
von Ubuntu 18.04 etwas verwirrt. Sie hatten ihn zuvor weder gesehen noch
genutzt. Alternativ könnte auch die Gruppe der Probanden so gewählt werden,
dass sie mit dem Betriebssystem bereits vertraut sind.
...
zu Beginn etwas schwieriger die gewünschte Information zu finden.
